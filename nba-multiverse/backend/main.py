...
from llm_engine import generate_branch_stream, generate_custom_branch_stream, _fallback_social_posts
from models import GeneratedBranch, GeneratedEvent, StatChange, SocialPost
from prompt_templates import validate_custom_text
...
def custom_choice_id(custom_text: str) -> str:
    normalized = custom_text.strip().lower()
    digest = hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:12]
    return f"custom_{digest}"


def _fork_is_unlocked(fid: str, state: WorldState) -> bool:
    idx = FORK_ORDER.index(fid) if fid in FORK_ORDER else -1
    if idx == -1:
        return False
    if idx > 0 and FORK_ORDER[idx - 1] not in state.completed_forks:
        return False
    ...
    return choices_satisfy_prerequisites(fork_prereqs, state.james_choices)
...
def _available_forks(state: WorldState) -> list[dict]:
    ...
    for i, fid in enumerate(FORK_ORDER):
        ...
        if not filtered and not _fork_is_unlocked(fid, state):
            continue
        result.append({
            "fork_id": fid,
...
class CustomChoiceBody(BaseModel):
    custom_text: str = Field(..., min_length=1, max_length=200)
...
async def record_choice(body: RecordChoiceBody):
    ...
    state = get_universe()
    is_custom = body.choice_id.startswith("custom_")
    ...
    elif not _fork_is_unlocked(body.fork_id, state):
        raise HTTPException(400, "This fork is not yet available")

    state = record_james_choice(
        state, body.fork_id, body.choice_id, body.choice_label[:80], body.timestamp,
    )
...
async def _stream_custom_branch(
    fork_id: str,
    choice_id: str,
    custom_text: str,
    choice_label: str,
    ...
    async for item in generate_custom_branch_stream(
        choice_id,
        custom_text,
        decision_dict,
        real_dict,
...
@app.post("/api/generate/{fork_id}/custom")
async def generate_custom_branch_fork(fork_id: str, body: CustomChoiceBody):
    """Stream-generate alternate timeline from free-form player text."""
    err = validate_custom_text(body.custom_text)
    if err:
    ...
    state = get_universe()
    if not _fork_is_unlocked(fork_id, state):
        raise HTTPException(400, "This fork is not yet available")

    choice_id = custom_choice_id(body.custom_text)
    choice_label = (body.label_hint or body.custom_text).strip()[:80]
    real = load_real_history(fork_id)
    cache_key = _cache_key(state.universe_id, fork_id, choice_id)

    async def stream():
        try:
            async for chunk in _stream_custom_branch(
                fork_id,
                choice_id,

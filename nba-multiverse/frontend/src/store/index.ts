...
export interface AppStore {
  ...
  setHighlightedForkId: (id: string | null) => void
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  hideMasterTailAfterFork: (forkId?: string) =>
    set((s) => {
      ...
      const hiddenIds = new Set(
        s.nodes
          .filter((n, i) => n.isRealHistory && n.type === 'event' && i > forkIdx)
          .map((n) => n.id),
      )
      return {
  ...
  pulsingPlayerIds: [],
  highlightedForkId: 'evt_lebron_2010',
  ...
  setHighlightedForkId: (id) => set({ highlightedForkId: id }),

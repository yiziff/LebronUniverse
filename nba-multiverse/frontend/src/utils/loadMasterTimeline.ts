...
import bundledMaster from '../../../data/master_timeline.json'

/** Full James master lane — matches NPC milestone span (2010–2020). */
const DEFAULT_MASTER_TIMELINE = bundledMaster as MasterTimelineData

export function resolveMasterTimeline(
  fromApi: MasterTimelineData | undefined | null,
): MasterTimelineData {
  if (!fromApi?.events?.length) return DEFAULT_MASTER_TIMELINE
  const apiEvents = fromApi.events.length
  const bundledEvents = DEFAULT_MASTER_TIMELINE.events.length
  const apiForks = fromApi.forks?.length ?? 0
  const bundledForks = DEFAULT_MASTER_TIMELINE.forks.length
  if (apiEvents >= bundledEvents && apiForks >= bundledForks) return fromApi
  return DEFAULT_MASTER_TIMELINE
}
...
export async function fetchUniverseData(): Promise<UniverseAPIResponse> {
  try {
    ...
    if (res.ok) {
      ...
      const data = (await res.json()) as UniverseAPIResponse
      data.master_timeline = resolveMasterTimeline(data.master_timeline)
      return data
    }
  ...
  const master = resolveMasterTimeline(
    legacy.master_timeline ?? {
